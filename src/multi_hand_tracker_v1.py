...
class MultiHandTracker():
    ...
    def _GetCandidateIdx(self, box_list, max_hands = max_hands_global, iou_thres = 0.45):
    ...
    def _merge_bbox_predicts(self, bbox_list, bbox_params):
        ...
        rearranged_params = [(None,None)]*self.max_hands
        leftover = curr_poly[:]
        for idx1, ppoly in enumerate(prev_poly):
            for idx2, cpoly in enumerate(curr_poly):
                if cpoly in leftover: continue #wont cpoly be always in leftover 
...
class MultiHandTracker3D():
    ...
    def _merge_bbox_predicts(self, bbox_list, bbox_params):
        ...
        rearranged_box = [None]*self.max_hands
        rearranged_params = [(None,None)]*self.max_hands #modified by Gokul - possible bug
        leftover = curr_poly[:]
        for idx1, ppoly in enumerate(prev_poly):
            for idx2, cpoly in enumerate(curr_poly):
